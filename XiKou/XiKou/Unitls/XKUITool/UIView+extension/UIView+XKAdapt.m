...
@implementation UIView (XKAdapt)

- (void)adaptScreenWidthWithType:(XKAdaptScreenWidthType)type
                     exceptViews:(NSArray<Class> *)exceptViews {
    if (type == XKAdaptScreenWidthTypeNone)  return;
    if (![self isExceptViewClassWithClassArray:exceptViews]) {
        
        // 是否要对约束进行等比例
        BOOL adaptConstraint = ((type & XKAdaptScreenWidthTypeConstraint) || type == XKAdaptScreenWidthTypeAll);
        
        // 是否对字体大小进行等比例
        BOOL adaptFontSize = ((type & XKAdaptScreenWidthTypeFontSize) || type == XKAdaptScreenWidthTypeAll);
        
        // 是否对圆角大小进行等比例
        BOOL adaptCornerRadius = ((type & XKAdaptScreenWidthTypeCornerRadius) || type == XKAdaptScreenWidthTypeAll);
