...
@property (nonatomic,strong)RACDisposable *xk_disposable;

@end

@implementation UITextField (XKUnitls)
...
- (void)setXk_monitor:(BOOL)xk_monitor{
    ...
    if (xk_monitor) {
        ...
        RACSignal *signal2 = [signal1 map:^id _Nullable(id  _Nullable value) {
            @strongify(self);
            NSString *text = [(NSString *)value deleteSpace];
